...
import { CheckCircle2, CircleDashed, Loader, CirclePlay } from "lucide-react";

const steps = [
  { name: 'Data Upload', status: 'completed' },
  { name: 'Preprocessing', status: 'completed' },
  { name: 'Train/Test Split', status: 'current' },
...
export function Status() {
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      case 'current':
        return <CirclePlay className="w-5 h-5 text-primary" />;
      case 'working':
